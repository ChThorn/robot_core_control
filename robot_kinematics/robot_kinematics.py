...
This version has kinematic parameters embedded to remove the URDF file dependency.
"""
...
class RobotKinematics:
    ...
    def __init__(self, ee_link: str = "tcp", base_link: str = "link0", constraints_path: Optional[str] = None):
        self.ee_link = ee_link # Informational only
        self.base_link = base_link # Informational only
        ...
        try:
            self.S, self.M, self.joint_limits = self._get_hardcoded_parameters()
        except Exception as e:
            raise RobotKinematicsError(f"Error loading hardcoded parameters: {e}")
        ...
        self._eye6 = np.eye(6)
        logger.info(f"Robot kinematics initialized with {self.n_joints} joints from hardcoded parameters.")
    ...
    def _get_hardcoded_parameters(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Home configuration (M): Transformation from the base to the end-effector
        # frame when all joint angles are zero.
        M = np.array([
            [1., 0., 0.,  0.0     ],
            [0., 1., 0., -0.00645],
            [0., 0., 1.,  0.8753  ],
    ...
    def _check_workspace(self, pos: np.ndarray) -> bool:
        ...
        # Convert constraint values from mm to meters for correct comparison
        x_min_m = ws["x_min"] / 1000.0
        x_max_m = ws["x_max"] / 1000.0
        y_min_m = ws["y_min"] / 1000.0
        y_max_m = ws["y_max"] / 1000.0
        z_min_m = ws["z_min"] / 1000.0
        z_max_m = ws["z_max"] / 1000.0

        return (x_min_m <= pos[0] <= x_max_m and
                y_min_m <= pos[1] <= y_max_m and
                z_min_m <= pos[2] <= z_max_m)
    ...
    def _matrix_log6(T: np.ndarray) -> np.ndarray:
        ...
        if abs(sin_th) < 1e-6:
            try:
                ...
                omega_unit = omega_unit / (np.linalg.norm(omega_unit) + 1e-12)
        ...
        omega_norm = np.linalg.norm(omega) + 1e-12
    ...
    def _ik_dls(self, T_des: np.ndarray, q0: np.ndarray, pos_tol: float, rot_tol: float, 
        ...
        phase_step_scales = [step_scale * 1.5, step_scale, step_scale * 0.5]
